...
import { editConference } from "@/functions/clickmeeting/editConference"
import { compareObjects, systemAction, systemActionStatus } from "@/functions/logging/actions"
import { systemLog } from "@/functions/logging/log"
import { addCourseWebinar, getCourse, updateCourse } from "@/functions/queries/course"
import validateSession from "@/functions/validateSession"
import { rm001000, rm001001, rm011000, rm011005, rm011012, rm011013 } from "@/responses/messages"
...
export async function PATCH(req: Request){
    ...
    if (course.place === "Online" && place !== "Online") { systemLog(systemAction.ADMeditCourse, systemActionStatus.error, rm011013, validatedUser, db); return unprocessableContent(rm011013) }
    ...
    if (changedCourse.webinar) {
        const changedConference = await editConference(changedCourse.webinar, changedCourse.title, changedCourse.instructor, changedCourse.date)
        if (changedConference){
            await addCourseWebinar(db, changedCourse.id, changedConference)
        }
    }
