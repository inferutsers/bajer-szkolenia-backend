...
export default async function signupForNewsletter(db: Pool, email: string): Promise<{success: boolean, isPresent: boolean}>{
    const isThereEmailLikeThis = await getNewsletterUserPresenceByEmail(db, email)
    if (isThereEmailLikeThis) { return {success: false, isPresent: true} }
    ...
    if (!newsletterUser || newsletterUser.rowCount == 0) { return { success: false, isPresent: false }}
