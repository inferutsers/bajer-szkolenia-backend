...
export async function getCourses(db: Pool): Promise<courseElement[] | undefined>{
    const courses = await db.query('SELECT "id", "date", "title", "place", "instructor", "note", "price", "span", "slots", "available", "dateCreated", "fileName" FROM "courses" WHERE date > $1 ORDER BY date', [getDateLong()])
...
export async function getRecentCourses(db: Pool): Promise<courseElement[] | undefined>{
    const courses = await db.query('SELECT "id", "date", "title", "place", "instructor", "note", "price", "span", "slots", "available", "dateCreated", "fileName" FROM courses WHERE date > $1 ORDER BY "dateCreated" DESC LIMIT 4', [getDateLong()])
    if (!courses || courses.rowCount == 0) { return undefined }
    const coursesFormatted: courseElement[] = await Promise.all(courses.rows.map(async (result) => await formatAsCourseElement(result, db)))
    return coursesFormatted
}
...
export async function getCourse(db: Pool, id: number | string): Promise<courseElement | undefined>{
    const courses = await db.query('SELECT "id", "date", "title", "place", "instructor", "note", "price", "span", "slots", "available", "dateCreated", "fileName" FROM "courses" WHERE date > $1 AND id = $2 LIMIT 1', [getDateLong(), id])
    if (!courses || courses.rowCount == 0) { return undefined}
...
export async function ADMgetCourse(db: Pool, id: number | string): Promise<ADMcourseElement | undefined>{
    const course = await db.query('SELECT "id", "date", "title", "place", "instructor", "note", "price", "span", "slots", "available", "dateCreated", "fileName" FROM "courses" WHERE date > $1 AND id = $2 LIMIT 1', [getDateLong(), id])
    if (!course || course.rowCount == 0) { return undefined }
...
export async function formatAsCourseElement(row: any, db: Pool): Promise<courseElement>{
    return { id: row.id, date: row.date, span: row.span, price: row.price, title: row.title, place: row.place, instructor: row.instructor, note: row.note, slots: row.slots, slotAvailable: await getSlotAvailability(db, row.id, row.slots), available: row.available, dateCreated: row.dateCreated, fileName: row.fileName }
}

export async function formatAsADMCourseElement(row: any, db: Pool): Promise<ADMcourseElement>{
    return { id: row.id, date: row.date, span: row.span, price: row.price, title: row.title, place: row.place, instructor: row.instructor, note: row.note, slots: row.slots, slotsUsed: await getCourseSignupCount(db, row.id), available: row.available, dateCreated: row.dateCreated, fileName: row.fileName }
}